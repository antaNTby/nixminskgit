...
$CONFIG = '{"lang":"by","error_reporting":true,"show_hidden":false,"hide_Cols":false,"theme":"dark"}';
 ...
 * @author tinyfilemanager.github.io
...
$auth_users = array(
    'root' => '$2y$10$s2mds.15.3HVT3p9vE95I.lk/S7yoqFZRxFAZNNrgB7z7Z6RnJuZi', // root6583031Admin
    'admin' => '$2y$10$9ZKQc9MD477.BcFhkHzqE.aBC6kj5UghbS8FtQMD/.CJySsJidsyO', //658!!!658!!!
    'user' => '$2y$10$RKs6yvSVgP41Ys3vqRQFZe5InnwzfL/ux/uTmD15b9YTktiH.Ywe2', //6583031
    'guest' => '$2y$10$7oz.1qSU86k7EWQTxYa/k.x/K4s7F6DCD5dZiocJtYkxXz1aAmgTC' //guest
);
...
$default_timezone = 'Europe/Minsk'; // UTC
...
-->
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="fm_usr" name="fm_usr" value="" required autofocus>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        <div class="alert alert-warning alert-dismissible fade show" role="alert">
                                          <strong>Login => passwords</strong><br> root, admin, user, guest => guest, sub
                        ...
                        <div class="footer text-center">
                            ...
                            &mdash;&mdash; &copy;
                            <a href="https://tinyfilemanager.github.io/" target="_blank" class="text-decoration-none text-muted" data-version="<?php echo VERSION; ?>">tinyfilemanager.github.io</a> &mdash;&mdash;
...
// Selectfile
/*if (isset($_POST['selectfile_from'], $_POST['selectfile_to'], $_POST['token']) && !FM_READONLY) {
    if(!verifyToken($_POST['token'])) {
        fm_set_msg("Invalid Token.", 'error');
    }
    // old name
    $old = urldecode($_POST['selectfile_from']);
    $old = fm_clean_path($old);
    $old = str_replace('/', '', $old);
    // new name
    $new = urldecode($_POST['selectfile_to']);
    $new = fm_clean_path(strip_tags($new));
    $new = str_replace('/', '', $new);
    // path
    $path = FM_ROOT_PATH;
        ...
        <div class="col-12">
            ...
            <div class="d-flex align-items-center mb-3">
                ...
                <?php } ?>
                    ...
                    <b class="ms-5">
                    <a class="btn btn-outline-primary px-2 text-bg-primary" style="font-size: 14px;" title="<?php echo lng('SelectLink') ?>"
                     data-href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $file) ?>"
                     onclick="selectfile('<?php echo fm_enc(addslashes(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : ''))) ?>', '<?php echo fm_enc(addslashes($file)) ?>');return false;"><i class="fa fa-thumb-tack"></i> <?php echo lng('SelectLink') ?></a>
                    </b>
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions">
                        ...
                        <a class="" title="<?php echo lng('SelectLink') ?>" data-href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" onclick="selectfile('<?php echo fm_enc(addslashes(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : ''))) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-thumb-tack px-3 mb-3 text-bg-primary"></i></a>
                    </td>
                </tr>
                <?php
                flush();
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-9">
            <ul class="list-inline footer-action">
                ...
                <a title="<?php echo lng('Backward') ?>" class="navbar-brand ps-lg-5 text-danger" href="javascript:window.history.back()"><i class="fa fa-arrow-left" aria-hidden="true"></i> <?php echo lng('Backward') ?></a>
            ...
            <div class="col-xs-6 col-sm-7">
                <ul class="navbar-nav justify-content-end <?php echo fm_get_theme();  ?>">
                    ...
                    <?php else: ?>
                        <?php if (!FM_READONLY): ?>
                            <li class="nav-item">
                                <a title="<?php echo lng('Settings') ?>" class="dropdown-item nav-link text-red" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;settings=1"><i class="fa fa-cog" aria-hidden="true"></i> <?php echo lng('Settings') ?></a>
                            </li>
                        <?php endif; ?>
                    <?php endif; ?>
...
<head>
    ...
    <meta name="author" content="tinyfilemanager.github.io">
...
<head>
    ...
    <style>
        ...
        *,*::before,*::after { box-sizing: border-box;}
        ...
        form.dropzone { min-height:200px;border:2px dashed #5a77a1;line-height:6rem; }
        ...
        .inline-actions > button > i { font-size:1em;margin-left:5px;background:#3785c1;color:#fff;padding:3px 4px;border-radius:3px; }
        ...
        i.fa.fa-file-code-o { color:#5a77a1 }
        i.fa.fa-code { color:#cc4b4c }
        i.fa.fa-file-text-o { color:#0096e6 }
        i.fa.fa-html5 { color:#d75e72 }
        i.fa.fa-file-excel-o { color:#09c55d }
        i.fa.fa-file-powerpoint-o { color:#f6712e }
        i.go-back { font-size:1.2em;color:#5a77a1; }
        ...
        .lds-facebook div { position:absolute;left:6px;width:13px;background:#5a77a1;animation:lds-facebook 1.2s cubic-bezier(0,.5,.5,1) infinite }
...
<div id="wrapper" class="container-fluid">
    ...
    <!-- selectfile Modal -->
    <div class="modal modal-alert" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" id="dialogPinFile">
      <div class="modal-dialog" role="document">
        <form class="modal-content rounded-3 shadow <?php echo fm_get_theme(); ?>" method="post" autocomplete="off">
          <div class="modal-body p-4 text-start">
            <h5 class="mb-3"><?php echo lng('Are you sure want to selectfile?') ?></h5>
            <p class="mb-1">
                <label class="form-label" for="js-selectfile-filename"><?php echo lng('File') ?></label>
                <input type="text" name="selectfile_filename" id="js-selectfile-filename" class="form-control mb-3" placeholder="<?php echo lng('Enter new file name') ?>" readonly required>
                <label class="form-label" for="js-selectfile-path"><?php echo lng('Path') ?></label>
                <input type="text" name="selectfile_path" id="js-selectfile-path" class="form-control mb-3" readonly required>
                <label class="form-label" for="token"><?php echo lng('Token') ?></label>
                <input type="text" name="token" id="js-token" value="<?php echo $_SESSION['token']; ?>" class="form-control mb-3" readonly required>
            <button class="btn btn-sm btn-danger col-8 m-2" onclick="localStorageRemoveItem();return false;"><strong><?php echo lng('Clear Item') ?></strong></button>
            </p>
          </div>
          <div class="modal-footer flex-nowrap p-0">
            <button type="button" class="btn btn-lg btn-link fs-6 text-decoration-none col-6 m-0 rounded-0 border-end" data-bs-dismiss="modal"><?php echo lng('Cancel') ?></button>
            <button class="btn btn-lg btn-link fs-6 text-decoration-none col-6 m-0 rounded-0" onclick="localStorageSetItem();return false;"><strong><?php echo lng('OK') ?></strong></button>
          </div>
        </form>
      </div>
    </div>
...
<script>
    ...
    function rename(e, t) {console.log(e,t);  if(t) { $("#js-rename-from").val(t);$("#js-rename-to").val(t); $("#renameDailog").modal('show'); } }
    //ant
    function selectfile(e, t) {
        // console.log(e,t);
        if(t) {
            $("#js-selectfile-filename").val(t);
            $("#js-selectfile-path").val(e);
            $("#dialogPinFile").modal('show');
         }
    }
    function localStorageSetItem(){
        let filename=document.getElementById('js-selectfile-filename').value;
        let path=document.getElementById('js-selectfile-path').value;
        let token=document.getElementById('js-token').value;
        localStorage.setItem('tfm_selectfile', filename); //сохраняем filename
        localStorage.setItem('tfm_path', path); //сохраняем path
        localStorage.setItem('tfm_token', token); //сохраняем token
        $("#dialogPinFile").modal('hide');
    }
