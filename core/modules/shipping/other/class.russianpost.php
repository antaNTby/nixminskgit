...
class RussianPost extends ShippingRateCalculator {
    ...
    public function allow_shipping_to_address( $_Address ) {
        ...
        if ( (int)$_Address["countryID"] != $this->getModuleSettingValue( 'CONF_RUSSIANPOST_COUNTRY' ) ) {
            return 0;
        }

        if ( !(int)$_Address["zoneID"] ) {
            return 0;
        }

        if ( !ZoneBelongsToCountry( $_Address["zoneID"], $_Address["countryID"] ) ) {
            return 0;
        }
    ...
    public function calculate_shipping_rate( $_Order, $_Address, $_ServID = 0 ) {
        ...
        if ( !isset( $ZoneNumber ) || !intval( $ZoneNumber ) || $ZoneNumber > 5 ) {
            return -1;
        }
        ...
        if ( $Weight > $this->getModuleSettingValue( 'CONF_RUSSIANPOST_MAX_WEIGHT' ) ) {
            return -1;
        }

        $GroundCost = $this->getModuleSettingValue( 'CONF_RUSSIANPOST_HALFCOST_' . $ZoneNumber ) + $this->getModuleSettingValue( 'CONF_RUSSIANPOST_OVERHALFCOST_' . $ZoneNumber ) * ceil(  (  ( $Weight < 0.5 ? 0.5 : $Weight ) - 0.5 ) / 0.5 );

        $AirCost = $GroundCost + $this->getModuleSettingValue( 'CONF_RUSSIANPOST_AIR' );

        if ( $Weight > $this->getModuleSettingValue( 'CONF_RUSSIANPOST_DIFFICULT_WEIGHT' ) || $this->getModuleSettingValue( 'CONF_RUSSIANPOST_CAUTION' ) ) {

            $GroundCost *= 1.3;
            $AirCost *= 1.3;
        }

        $OrderAmount = $this->_convertCurrency( $_Order['order_amount'], 0, $this->getModuleSettingValue( 'CONF_RUSSIANPOST_CURRENCY' ) );

        $GroundCost += $OrderAmount * ( $this->getModuleSettingValue( 'CONF_RUSSIANPOST_COMMISION' ) / 100 );
        $AirCost += $OrderAmount * ( $this->getModuleSettingValue( 'CONF_RUSSIANPOST_COMMISION' ) / 100 );
        ...
        $Rates[] = array(
            ...
            'id'   => 1,
            'rate' => $this->_convertCurrency( $GroundCost, $this->getModuleSettingValue( 'CONF_RUSSIANPOST_CURRENCY' ), 0 ),
        );
        $Rates[] = array(
            'name' => 'Авиа',
            'id'   => 2,
            'rate' => $this->_convertCurrency( $AirCost, $this->getModuleSettingValue( 'CONF_RUSSIANPOST_CURRENCY' ), 0 ),
        );
    ...
    public function initSettingFields() {
        ...
        $this->SettingsFields['CONF_RUSSIANPOST_ZONES'] = array(
            ...
            'settings_html_function' => 'RussianPost::settingZones(' . $this->ModuleConfigID . ', "' . $this->getModuleSettingRealName( 'CONF_RUSSIANPOST_COUNTRY' ) . '")',
            'sort_order'             => 30,
        );
    ...
    public function settingHalfCosts( $_ID, $_SettingID ) {

        static $CostCounters = array();
        static $CostHTML     = array();

        if ( !isset( $CostCounters[$_ID] ) ) {
            $CostCounters[$_ID] = '';
        }

        if ( !isset( $CostHTML[$_ID] ) ) {
            $CostHTML[$_ID] = '';
        }

        $CostCounters[$_ID]++;
        $CostHTML[$_ID] .= '<tr><td>Пояс ' . $CostCounters[$_ID] . ' </td><td>' . setting_TEXT_BOX( 0, $_SettingID ) . '</td></tr>';
        if ( $CostCounters[$_ID] == 5 ) {
            return '<table>' . $CostHTML[$_ID] . '</table>';
        } else {
            return -1;
        }
    ...
    public function settingZones( $_ModuleConfigID, $_CountryConstant ) {
        ...
        foreach ( $Zones as $_Zone ) {
            ...
            if ( !isset( $ZoneNumbers[$_Zone['zoneID']] ) ) {
                $ZoneNumbers[$_Zone['zoneID']] = '';
            }
