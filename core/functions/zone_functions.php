...
function znGetZones( $countryID = null ) {
    if ( $countryID == null ) {
        $q = db_query( "select zoneID, zone_name, " .
            ...
            " order by zoneID, zone_name" );
    } else {
        $q = db_query( "select zoneID, zone_name, " .
            " zone_code, countryID from " . ZONES_TABLE . " " .
            " where countryID=" . (int)$countryID . " order by zoneID, zone_name" );
    }
...
function znDeleteZone( $zoneID ) {
    ...
    foreach ( $tax_classes as $classr ) {
        taxDeleteZoneRate( $classr["classID"], $zoneID );
    }
