...
function settingQUICK_CART_COLNUM() {
    ...
    $res        = "<select class='form-select' name='settingQUICK_CART_COLNUM'>";
...
function _getSettingOptionValue( $settings_constant_name ) {
    global $conf_data;
    if ( isset( $conf_data[$settings_constant_name] ) ) {
        return $conf_data[$settings_constant_name];
    }
...
function settingCallHtmlFunction( $constantName ) {
    $sql="SELECT settings_html_function, settingsID, settings_constant_name FROM " .
...
function settingDefineConstants() {
    ...
    while ( $row = db_fetch_row( $q ) ) {
        ...
        if ( !defined( $row["settings_constant_name"] ) ) {
            define( $row["settings_constant_name"], $row["settings_value"] );
        }
    }
...
<div class=\"form-check\">
  <input class=\"form-check-input\" type=\"checkbox\" value=\"\" id=\"flexCheck_{$settings_constant_name}\" name=\"setting{$settings_constant_name}\"{$isChecked}>
  <label class=\"form-check-label\" for=\"flexCheck_{$settings_constant_name}\">
    <var class=text-danger>{$settings_constant_name}</var>
  </label>
</div>
";
...
function setting_TEXT_BOX( $dataType, $settingsID, $BlockInSafeMode = null ) {
    ...
    if ( isset( $BlockInSafeMode ) ) {
        ...
        if ( $settingsID && CONF_BACKEND_SAFEMODE ) {
            return ADMIN_SAFEMODE_BLOCKED;
        } else {
    ...
    if ( isset( $_POST["save"] ) && isset( $_POST["setting" . $settings_constant_name] ) ) {
        if ( $dataType == 0 ) {
            ...
            $valueNF = (float)$_POST["setting" . $settings_constant_name];
            $value   = number_format( $valueNF, 4, ".", "" );
        } elseif ( $dataType == 2 ) {
    ...
    switch ( $dataType ) {
        case 0:
            $input_type = "type='text' class='form-control font-monospace text-start'";
            break;
        case 1:
            $input_type = "type='number' class='form-control font-monospace text-end' step=0.01";
            break;
        case 2:
            $input_type = "type='number' class='form-control font-monospace text-end' step=1";
            break;
        default:
            $input_type = "type='text' class='form-control font-monospace text-start'";
            break;
    ...
    $res .= "<input {$input_type} value='" . _getSettingOptionValue( $settings_constant_name ) .
        "' name='setting" . $settings_constant_name . "' id='setting" . $settings_constant_name . "' >";
    $res .= "<small class='ps-2 mt-0 text-muted text-opacity-25'>" . _getSettingOptionValue( $settings_constant_name ) . "</small>";
...
function setting_TEXT_BOX_SECURE( $dataType, $settingsID ) {
    ...
    return "<input type='text' class='form-control font-monospace text-start' value='" . cryptCCNumberDeCrypt( _getSettingOptionValue( $settings_constant_name ), NULL ) .
...
function setting_DATEFORMAT() {
    ...
    if ( !strcmp( $current_format, "DD.MM.YYYY" ) ) {
        $res .= " selected";
    }
    $res .= ">DD.MM.YYYY</option>";

    //third option - YYYY-MM-DD - SQL style
    $res .= "<option value='YYYY-MM-DD'";
    if ( !strcmp( $current_format, "YYYY-MM-DD" ) ) {
        $res .= " selected";
    }
    $res .= ">YYYY-MM-DD</option>";
...
function setting_WEIGHT_UNIT( $settingsID ) {
    ...
    $res = "<select class='form-select' name='setting_WEIGHT_UNIT'>";
    ...
    foreach ( $units as $key => $val ) {
        ...
        if ( !strcmp( _getSettingOptionValue( "CONF_WEIGHT_UNIT" ), $key ) ) {
            $res .= " selected ";
        }
...
function settingCONF_DEFAULT_CURRENCY() {
    ...
    $res        = "";
    $currencies = currGetAllCurrencies();
    $res        = "<select class='form-select' name='settingCONF_DEFAULT_CURRENCY'>";
...
function settingCONF_MAIL_METHOD() {
    ...
    $res        = "";
    $res        = "<select class='form-select' name='settingCONF_MAIL_METHOD'>";
    $res .= "<option value='0'";
    if ( $selectedID == 0 ) {
        $res .= " selected ";
    }
...
function settingCONF_USER_SYSTEM() {
    ...
    $res        = "";
    $res        = "<select class='form-select' name='settingCONF_USER_SYSTEM'>";
    $res .= "<option value='0'";
    if ( $selectedID == 0 ) {
        $res .= " selected ";
    }
...
function settingCONF_TIMEZONE() {
    ...
    $res        = "";
    $res        = "<select class='form-select' name='settingCONF_TIMEZONE' data-size='6'>";
...
function settingCONF_DEFAULT_COUNTRY() {
    ...
    $res = "<select class='form-select' name='settingCONF_DEFAULT_COUNTRY'>";
    $res .= "<option value='0'>" . ADMIN_NOT_DEFINED . "</option>";
...
function settingCONF_DEFAULT_TAX_CLASS() {
    ...
    $res = "<select class='form-select' name='settingCONF_DEFAULT_TAX_CLASS'>";
    $res .= "        <option value='0'>" . ADMIN_NOT_DEFINED . "</option>";
...
function settingCONF_DEFAULT_TEMPLATE() {
    ...
    $res        = "<select class='form-select' name='settingCONF_DEFAULT_TEMPLATE'>";
...
function settingCONF_SELECT_CART_METHOD() {
    ...
    $res          = "<select class='form-select' name='settingCONF_SELECT_CART_METHOD'>";
    ...
    for ( $i = 0; $i < count( $methodlist ); $i++ ) {
        if ( $methodlist[$i] != "" ) {
            ...
            if ( $methodlist[$i]["value"] == $selectedID ) {
                $res .= "selected";
            }
...
function settingCONF_DEFAULT_SORT_ORDER() {
    ...
    $res         = "<select class='form-select' name='settingCONF_DEFAULT_SORT_ORDER'>";
    ...
    for ( $i = 0; $i < count( $sortlist ); $i++ ) {
        if ( $sortlist[$i] != "" ) {
            ...
            if ( $sortlist[$i]["value"] == $selectedsID ) {
                $res .= "selected";
            }
...
function settingCONF_DISPLAY_FOTO() {
    ...
    $res         = "<select class='form-select' name='settingCONF_DISPLAY_FOTO'>";
    $selectedsID = _getSettingOptionValue( "CONF_DISPLAY_FOTO" );
    $sortlist    = array();
    $sortlist[]  = array( "title" => BLOCKS_RIGHT, "value" => 0 );
    $sortlist[]  = array( "title" => BLOCKS_LEFT, "value" => 1 );
...
function settingCONF_DEFAULT_CUSTOMER_GROUP() {
    ...
    $res        = "<select class='form-select' name='settingCONF_DEFAULT_CUSTOMER_GROUP'>";
...
function settingCONF_NEW_ORDER_STATUS() {
    ...
    $orders = ostGetOrderStatuses( false );
    ...
    if ( count( $orders ) < 2 ) {
        ...
        $res .= "<select class='form-select' name='settingCONF_NEW_ORDER_STATUS'>\n";
...
function settingCONF_ACTIVE_ORDER_STATUS() {
    if ( isset( $_POST["save"] ) && isset( $_POST["settingCONF_ACTIVE_ORDER_STATUS"] ) ) {
        _setSettingOptionValue( "CONF_ACTIVE_ORDER_STATUS",
            $_POST["settingCONF_ACTIVE_ORDER_STATUS"] );
    }
    ...
    if ( count( $orders ) < 2 ) {
        ...
        $selectedID = _getSettingOptionValue( "CONF_ACTIVE_ORDER_STATUS" );
        if ( $selectedID == "" ) {
            $res .= "<b>" . ADMIN_STATUS_IS_NOT_DEFINED . "</b>&nbsp;";
        }

        $res .= "<select class='form-select' name='settingCONF_ACTIVE_ORDER_STATUS'>\n";
        foreach ( $orders as $order ) {
            $res .= "<option value='" . $order["statusID"] . "' ";
            if ( $selectedID == $order["statusID"] ) {
                $res .= "selected";
...
function settingCONF_COMPLETED_ORDER_STATUS() {
    ...
    if ( count( $orders ) < 2 ) {
        ...
        $res .= "<select class='form-select' name='settingCONF_COMPLETED_ORDER_STATUS'>\n";
        foreach ( $orders as $order ) {
            $res .= "<option value='" . $order["statusID"] . "' ";
            if ( $selectedID == $order["statusID"] ) {
                $res .= "selected";
...
function setting_ORDER_STATUS_SELECT( $_SettingID ) {
    ...
    $statuses = ostGetOrderStatuses( false );
...
function settingCONF_COUNTRY() {
    ...
    // onchange='$onChange'
    $res .= "<select class='form-select' name='settingCONF_COUNTRY' >\n";
    $res .= "        <option value='0'>" . ADMIN_NOT_DEFINED . "</option>";
    foreach ( $countries as $country ) {
        $res .= "<option value='" . $country["countryID"] . "' ";
        if ( $selectedID == $country["countryID"] ) {
...
function settingCONF_ZONE() {
    ...
    if ( count( $countries ) != 0 ) {
        ...
        if ( count( $zones ) > 0 ) {
            $res .= "<select class='form-select' name='settingCONF_ZONE'>\n";
...
function settingCONF_CALCULATE_TAX_ON_SHIPPING() {
    ...
    $res = "<select class='form-select' name='settingCONF_CALCULATE_TAX_ON_SHIPPING'>";
    $res .= "        <option value='0'>" . ADMIN_NOT_DEFINED . "</option>";
...
function setting_SELECT_BOX( $_Options, $_SettingID ) {
    ...
    $html                 = '<select class=\'form-select\' name="setting_' . $settings_constant_name . '">';
...
function setting_SELLER_SELECT( $_SettingID ) {

    $Options = array( array( 'title' => ADMIN_NOT_DEFINED, 'value' => 0 ) );
    // $Companies = companGetAllCompanies();
    $Companies = companGetAllCompaniesGrouped( "company_unp" );
    foreach ( $Companies as $_company ) {

        // $Options[] = array(
        //     'title' => "<sub>{$_company['companyID']}</sub> {$_company['company_name']}",
        //     'value' => $_company['companyID'],
        //     'sub'   => htmlCompanyPreview( $_company, 26 ),
        // );

        $Options2[] = array(
            "title"   => "{$_company["company_name"]} / УНП{$_company["company_unp"]}",
            "value"   => $_company["companyID"],
            // "selected" => ( $_company["companyID"] == $_SESSION["companyID"] ) ? 1 : 0,
             "subtext" => "id{$_company["companyID"]} {$_company["update_time"]} {$_company["company_adress"]}",
            "tokens"  => "{$_company["companyID"]} {$_company["company_name"]} {$_company["company_unp"]} {$_company["company_adress"]} {$_company["company_bank"]}",
        );
    }

    // return setting_SELECTPICKER_BOX( $Options, $_SettingID, "custord", "companies" );
    return setting_SELECTPICKER_BOX( $Options2, $_SettingID, "custord", "companies" );
}

function settingCONF_STAMP_CONTRACT_ID( $_SettingID ) {
    $Options = array( array( 'title' => ADMIN_NOT_DEFINED, 'value' => 0 ) );
    ...
    foreach ( $Contracts as $_contract ) {
        ...
        $Options[] = array(
            'title' => ( $_contract['contract_title'] ),
            'value' => $_contract['contractID'],
            'sub'   => $_contract['update_time'] . '<sub>[ ' . $_contract['contractID'] . ']</sub>',
        );
    }

    return setting_SELECTPICKER_BOX( $Options, $_SettingID, "custord", "contracts" );
}

function setting_SELECTPICKER_BOX( $_Options, $_SettingID, $dpt, $sub ) {
    ...
    if ( !is_array( $_Options ) ) {
        ...
        for ( ; $TC >= 0; $TC-- ) {
            ...
            $_Options[$TC]          = explode( ':', $_Options[$TC] );
            $_Options[$TC]['title'] = $_Options[$TC][0];
            $_Options[$TC]['sub']   = $_Options[$TC][2];
            $_Options[$TC]['id']    = $_Options[$TC][3];
            if ( !isset( $_Options[$TC][1] ) ) {
                $_Options[$TC]['value'] = '';
                $_Options[$TC]['sub']   = '';
            } else {
    ...
    $SettingconstantValue = _getSettingOptionValue( $settings_constant_name );
    $alink                = ADMIN_FILE . '?dpt=' . $dpt . '&sub=' . $sub . '&edit_mode=' . (int)$SettingconstantValue;

    // $options_html = '';
    // foreach ( $_Options as $_Option ) {
    //     $options_html .= '<option data-content="' . $_Option['title'] . "&nbsp;<small class=text-muted>" . $_Option['sub'] . '</sub>" value="' . $_Option['value'] . '"' . ( $SettingconstantValue == $_Option['value'] ? ' selected="selected"' : '' ) . '>' . '</option>';
    // }

    $options_html2 = '';
    foreach ( $_Options as $_Option ) {
        $is_selected = ( $SettingconstantValue == $_Option['value'] ? " selected=\"selected\"" : "" );
        $options_html2 .= "
        <option class=\"text-wrap px-auto\" value=\"{$_Option['value']}\"
        data-tokens=\"{$_Option['tokens']}\" data-subtext=\"{$_Option['subtext']}\"
        {$is_selected}/>
        {$_Option['title']}
        </option>
        ";
    ...
    $html .= '<select id="setting_' . (int)$_SettingID . '" class=\'form-select selectpicker show-tick \' name="setting_' . $settings_constant_name . '" data-live-search="true" data-size="10"  data-width="80%" onChange="SettingsSelect_change(' . (int)$_SettingID . ', \'' . $dpt . '\', \'' . $sub . '\');return false;">';
    $html .= $options_html;
    $html .= '</select>';
    $html .= '    </div>'; //form-group
    $html .= '  </div>';   //col-sm-12
    $html .= '  <div class="col-sm-12">';
    $html .= '<a id ="btn_setting_' . (int)$_SettingID . '" class="btn btn-light btn-sm" role="button" href="' . $alink . '">';
    $html .= '<i class="bi bi-gear-wide"></i> edit</a>';
    $html .= '  </div>'; //col-sm-3
    $html .= '</div>';   //row
    ...
    <div class=\"form-group row\">
      <label for=\"setting_{$_SettingID}\" class=\"col-lg-1 control-label\"><a id =\"btn_setting_{$_SettingID}\" class=\"btn btn-link\" role=\"button\" href=\"{$alink}\"><i class=\"bi bi-gear-wide-connected\"></i></a></label>
    <div class=\"col-lg-11\">
    <select
    class=\"selectpicker show-tick form-control\"
    id=\"setting_{$_SettingID}\"
    name=\"setting_{$settings_constant_name}\"
    data-live-search=\"true\"
    max-width=
    onChange=\"SettingsSelect_change( {$_SettingID}, '{$dpt}' , '{$sub}');return false;\"
    />
        {$options_html2}
    </select>
